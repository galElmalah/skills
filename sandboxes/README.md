...
These projects exist to test the `experiments` skill against controlled, measurable problems.

## Included Projects

| Project | Suggested metric | Direction | Benchmark setup |
| --- | --- | --- | --- |
| [catalog-project](./catalog-project) | `total_duration_ms` | `min` | Create the cheap benchmark with `experiments`. |
| [tests-project](./tests-project) | `test_runtime_ms` | `min` | Create the cheap benchmark with `experiments`. |
...
1. Pick one sandbox.
2. Use the `experiments` skill to scaffold an experiment workspace inside that sandbox.
